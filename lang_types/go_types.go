...
func GoDocComment(entity pgs.Entity) string {
	...
	return fmt.Sprintf("// %s %s", entity.Name().UpperCamelCase(), strings.Join(commentLines, "\n//"))
}
...
{{ range .File.Services }}{{ range .Methods }}
    {{ GolangDocComment . }} 
    {{ .Name.UpperCamelCase }}(userContext context.Context, request *{{ GolangMethodParamType . }}) ({{ GolangMethodReturnType . }}, error){{ end }}
{{ end }}
